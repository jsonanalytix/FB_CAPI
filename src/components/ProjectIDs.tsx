  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">
            Tagging Server URL <span className="text-red-500">*</span>
          </label>
          <input
            type="url"
            value={config.taggingServerUrl}
            onChange={(e) => updateField('taggingServerUrl', e.target.value)}
            placeholder="https://tags.client.com"
            className={`w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 transition-colors ${
              config.taggingServerUrl && !isValidUrl(config.taggingServerUrl)
                ? 'border-red-300 focus:border-red-500 focus:ring-red-200'
                : 'border-gray-300 focus:border-blue-500 focus:ring-blue-200'
            }`}
          />
